...
test("legacy snapshot preserves unknown explicit providers instead of mislabeling them", () => {
  const snapshot = deriveLegacyUiSnapshot({
    loginChecked: true,
    sessions: [{
      id: "future-provider",
  ...
  assert.equal(snapshot.sidebar.groups[0].entries[0]?.provider, "future-cli");
});
...
test("every UiAction maps through the narrow legacy command port", async () => {
  ...
  const actions: UiAction[] = [
    ...
    { type: "nav.refresh" },
    { type: "session.new" },
    { type: "session.newAt", cwd: "/workspace/project" },
  ...
  assert.deepEqual(calls, [
    ...
    ["refreshPage"],
    ["openNewSession"],
    ["openNewSession", "/workspace/project"],
