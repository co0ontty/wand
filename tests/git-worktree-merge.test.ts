...
test("mergeSessionWorktreeAsync keeps the main checkout untouched when preflight finds conflicts", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => mergeSessionWorktreeAsync({ worktree: prepared.worktree, targetBranch: "main" }),
      (error: unknown) => {
...
test("mergeSessionWorktreeAsync restores branch and HEAD after a merge hook times out", async () => {
...
test("mergeSessionWorktreeAsync preserves successful merge and cleanup semantics", async () => {
