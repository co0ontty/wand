...
(() => {
  ...
  function openFilePreview(filePath) {
    ...
    if (!overlay) {
      ...
      overlay.innerHTML = '<div class="file-preview-modal" tabindex="-1"><div class="file-preview-header"><div class="file-preview-title"><span class="file-preview-icon">\u{1F4C4}</span><div class="file-preview-name-block"><div class="file-preview-name-row"><span class="file-preview-filename">\u52A0\u8F7D\u4E2D\u2026</span></div><span class="file-preview-path" title=""></span></div></div><div class="file-preview-toolbar"></div><button class="file-preview-close" title="\u5173\u95ED (Esc)" aria-label="\u5173\u95ED">' + wandFileIcon("x", { size: 18 }) + '</button></div><div class="file-preview-body"><div class="file-preview-loading">\u52A0\u8F7D\u9884\u89C8\u2026</div></div></div>';
      ...
      if (titleEl) {
        titleEl.innerHTML = '<span class="file-preview-icon">\u{1F4C4}</span><div class="file-preview-name-block"><div class="file-preview-name-row"><span class="file-preview-filename">\u52A0\u8F7D\u4E2D\u2026</span></div><span class="file-preview-path" title=""></span></div>';
      }
  ...
  function renderSessionItem(session) {
    ...
    var titleHtml = session.summary ? '<div class="session-title">' + escapeHtml2(session.summary) + "</div>" : '<div class="session-command">' + escapeHtml2(session.resumedFromSessionId ? session.command.replace(/\s+--resume\s+\S+/, "").replace(/\s+resume\s+[0-9a-f-]+/, "") : session.command) + "</div>";
    ...
    return '<div class="session-item' + activeClass + selectedClass + '" data-session-id="' + session.id + '" role="button" tabindex="0"><div class="session-item-content"><div class="session-item-row">' + checkbox + '<div class="session-main"><div class="session-title-row">' + titleHtml + timeDisplay + "</div>" + activityHtml + '<div class="session-meta"><span class="session-status ' + metaStatusClass + '">' + escapeHtml2(metaStatus) + "</span>" + badgesHtml + recoveryHtml + "</div></div>" + actionsHtml + "</div></div></div>";
  }
  ...
  function getSessionKindHint(kind) {
    ...
    if (kind === "structured") {
      return tool === "codex" ? "Codex JSONL \u7ED3\u6784\u5316\u804A\u5929\u754C\u9762\uFF0C\u652F\u6301\u591A\u8F6E\u5BF9\u8BDD\u548C\u5DE5\u5177\u8C03\u7528\u5C55\u793A\u3002" : "\u7ED3\u6784\u5316\u804A\u5929\u754C\u9762\uFF0C\u652F\u6301\u591A\u8F6E\u5BF9\u8BDD\u3001\u6D41\u5F0F\u8F93\u51FA\u548C\u5DE5\u5177\u8C03\u7528\u5C55\u793A\u3002";
    }
    if (tool === "codex") {
  ...
  function renderSessionModal() {
    ...
    return '<section id="session-modal" class="modal-backdrop hidden"><div class="modal session-modal"><div class="modal-header"><div><h2 class="modal-title">\u65B0\u5BF9\u8BDD</h2><p class="modal-subtitle">\u542F\u52A8 Claude \u6216 Codex \u4F1A\u8BDD\uFF0C\u9009\u62E9 provider\u3001\u4F1A\u8BDD\u7C7B\u578B\u3001\u6A21\u5F0F\u548C\u5DE5\u4F5C\u76EE\u5F55\u3002</p></div><button id="close-modal-button" class="btn btn-ghost btn-icon modal-close-btn" type="button" aria-label="\u5173\u95ED"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" aria-hidden="true"><line x1="6" y1="6" x2="18" y2="18"/><line x1="18" y1="6" x2="6" y2="18"/></svg></button></div><div class="modal-body"><div class="field"><label class="field-label">Provider</label><div id="provider-cards" class="mode-cards">' + renderProviderOptions(modalTool) + '</div></div><div class="field"><label class="field-label">\u4F1A\u8BDD\u7C7B\u578B</label><div id="session-kind-cards" class="mode-cards">' + renderSessionKindOptions(sessionKind) + '</div><div class="field-hint session-kind-hint-row"><span id="session-kind-description">' + escapeHtml2(getSessionKindHint(sessionKind)) + '</span></div></div><div class="field"><label class="field-label">\u6A21\u5F0F</label><div id="mode-cards" class="mode-cards">' + renderModeCards(modalMode) + '</div><p id="mode-description" class="field-hint">' + escapeHtml2(getToolModeHint2(modalTool, modalMode)) + '</p></div><div class="field"><label class="field-label" for="cwd">\u5DE5\u4F5C\u76EE\u5F55</label><div class="suggestions-wrap"><input id="cwd" type="text" class="field-input" autocomplete="off" placeholder="' + escapeHtml2(getEffectiveCwd3()) + '" /><div id="cwd-suggestions" class="suggestions hidden"></div></div><p class="field-hint">\u7559\u7A7A\u5219\u4F7F\u7528\u4E0A\u65B9\u76EE\u5F55\uFF0C\u652F\u6301\u8DEF\u5F84\u81EA\u52A8\u8865\u5168\u3002</p><div id="recent-paths-bubbles" class="recent-paths-bubbles"></div></div></div><div class="modal-footer"><button id="run-button" class="btn btn-primary btn-block">\u542F\u52A8\u4F1A\u8BDD</button><p id="modal-error" class="error-message hidden"></p></div></div></section>';
  }
    ...
    switch (msg.type) {
      case "output":
        ...
        if (msg.data && msg.sessionId) {
          ...
          } else if (snapshot.output !== void 0 || snapshot.messages || isIncremental || msg.data.permissionBlocked !== void 0) {
            updateSessionSnapshot(snapshot);
  ...
  function attachEventListeners() {
      ...
      topbarMoreMenu.addEventListener("click", function(e) {
        ...
        switch (action) {
          case "copy-claude-session-id":
            copySelectedSessionField("claudeSessionId", getSelectedSession3() && getSelectedSession3().provider === "codex" ? "Codex thread ID \u5DF2\u590D\u5236" : "Claude \u4F1A\u8BDD ID \u5DF2\u590D\u5236");
            break;
          case "copy-cwd":
    ...
    function renderRecentPathsHtml(items) {
      ...
      items.forEach(function(item) {
        ...
        html += '<div class="folder-recent-item" data-path="' + escapeHtml2(p) + '"><span class="folder-recent-item-path">' + escapeHtml2(p) + "</span></div>";
      });
      html += "</div>";
      return html;
    }
  ...
  function renderDiffTool(block, toolResult, toolName, messageKey, index) {
    ...
    return '<div class="inline-diff' + collapsedClass + '" data-tool-name="' + escapeHtml2(toolName) + '" data-expand-kind="diff" data-expand-key="' + escapeHtml2(expandKey) + '" data-tool-use-id="' + escapeHtml2(toolId) + '"><div class="diff-header" onclick="__tcToggle(event,this)"><span class="diff-file-icon"></span><span class="diff-file-name">' + escapeHtml2(fileName) + '</span><span class="diff-path">' + escapeHtml2(path) + '</span><span class="diff-status ' + statusClass + '">' + statusText + '</span><span class="diff-toggle">\u25BC</span></div><div class="diff-body"><div class="diff-columns">' + (bothCols ? '<div class="diff-col ' + colClass + '"><div class="diff-col-label">\u65E7</div>' + leftCol + "</div>" : "") + '<div class="diff-col ' + colClass + '"><div class="diff-col-label">' + (bothCols ? "\u65B0" : "") + "</div>" + (rightCol || leftCol) + "</div></div></div></div>";
  }
  ...
  function shouldRequestChatFormat2(session) {
    if (!session) return false;
  ...
  var THINKING_LEVELS = [
    ...
    { id: "standard", label: "think", hint: "Claude CLI: think \xB7 SDK budget 4096 \xB7 Codex low" },
    { id: "deep", label: "think hard", hint: "Claude CLI: think hard \xB7 SDK budget 16000 \xB7 Codex medium" },
    { id: "max", label: "ultrathink", hint: "Claude CLI: ultrathink \xB7 SDK budget 31999 \xB7 Codex high" }
  ];
  ...
  function fetchAvailableModels() {
    return fetch("/api/models", { credentials: "same-origin" }).then(function(res) {
      ...
      if (data && Array.isArray(data.models)) {
        ...
        syncComposerModelSelect(getSelectedSession5());
  ...
  function loadOutput(id) {
    ...
    if (shouldRequestChatFormat2(sess)) {
      url += "?format=chat";
    }
  ...
  function loadSettingsData() {
    fetch("/api/settings", { credentials: "same-origin" }).then(function(res) {
      return res.json();
    }).then(function(data) {
  ...
  function quickStartSession() {
    ...
    ensureTerminalReady().then(function() {
      return fetch("/api/commands", {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "same-origin",
        body: JSON.stringify(withTerminalDimensions({ command, provider: command, cwd: defaultCwd, mode: defaultMode }))
      });
    }).then(function(res) {
      return res.json();
    }).then(function(data) {
  ...
  function runPtyCommandFromModal(command, cwd, mode, worktreeEnabled, errorEl) {
    ...
    ensureTerminalReady().then(function() {
      ...
      showError2(errorEl, command === "codex" ? "\u65E0\u6CD5\u542F\u52A8 Codex \u4F1A\u8BDD\uFF0C\u8BF7\u786E\u8BA4 codex \u5DF2\u6B63\u786E\u5B89\u88C5\u5E76\u53EF\u5728\u7EC8\u7AEF\u4E2D\u6267\u884C\u3002" : "\u65E0\u6CD5\u542F\u52A8 Claude \u4F1A\u8BDD\uFF0C\u8BF7\u786E\u8BA4 Claude \u5DF2\u6B63\u786E\u5B89\u88C5\u3002");
    }).finally(function() {
      _sessionCreating = false;
    });
  }
  ...
  function loadBlankChatCwdDropdown(dropdown) {
    ...
    fetchRecentPaths(function(items) {
      ...
      html += '<div class="blank-chat-cwd-item' + (currentDir === defaultCwd ? " active" : "") + '" data-path="' + escapeHtml2(defaultCwd) + '"><span class="blank-chat-cwd-item-label">\u9ED8\u8BA4</span><span class="blank-chat-cwd-item-path">' + escapeHtml2(defaultCwd) + "</span></div>";
      if (items.length) {
        var seen = {};
        ...
        items.forEach(function(item) {
          ...
          seen[item.path] = true;
          html += '<div class="blank-chat-cwd-item' + (currentDir === item.path ? " active" : "") + '" data-path="' + escapeHtml2(item.path) + '"><span class="blank-chat-cwd-item-path">' + escapeHtml2(item.path) + "</span></div>";
        });
      }
  ...
  function loadRecentPathBubbles() {
    ...
    fetchRecentPaths(function(items) {
      ...
      container.innerHTML = items.map(function(item) {
        return '<button class="recent-path-bubble" data-path="' + escapeHtml2(item.path) + '" title="' + escapeHtml2(item.path) + '">' + escapeHtml2(item.name) + "</button>";
      }).join("");
  ...
  function renderPathSuggestions(items) {
    ...
    container.innerHTML = items.map(function(item) {
      return '<button class="suggestion-item" data-path="' + escapeHtml2(item.path) + '"><strong>' + escapeHtml2(item.name) + "</strong><small>" + escapeHtml2(item.path) + "</small></button>";
    }).join("");
  ...
  function handleInputBoxKeydown(event) {
    ...
    if (event.key === "Escape") {
      event.preventDefault();
      var escSess = getSelectedSession5();
    ...
    if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "c") {
      ...
      var ccSess = getSelectedSession5();
    ...
    if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "d") {
      ...
      var cdSess = getSelectedSession5();
    ...
    if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "l") {
      event.preventDefault();
      var clSess = getSelectedSession5();
  ...
  if ("serviceWorker" in navigator) {
    fetch("/sw.js", { cache: "no-cache" }).then(function(response) {
      if (response.ok) {
