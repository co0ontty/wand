...
(() => {
  ...
  function attachEventListeners() {
    ...
    ["cfg-system-ai-base-url", "cfg-system-ai-model", "cfg-system-ai-key"].forEach(function(id) {
      ...
      if (field) field.addEventListener("input", function(e) {
        ...
        var key = document.getElementById("cfg-system-ai-key");
    ...
    if (systemAiProtocol) systemAiProtocol.addEventListener("change", function() {
      var key = document.getElementById("cfg-system-ai-key");
      var authHeader = document.getElementById("cfg-system-ai-auth-header");
      if (authHeader) authHeader.value = systemAiProtocol.value === "anthropic" ? "x-api-key" : "bearer";
      if (key) key.dataset.source = "custom";
      syncCommitSourceUI();
    });
    ...
    if (commitApiConfigure) commitApiConfigure.addEventListener("click", function() {
      var field = document.getElementById("cfg-system-ai-base-url");
      if (!field) return;
  ...
  function syncCommitSourceUI() {
    var apiRadio = document.getElementById("cfg-commit-source-api");
    var cliRadio = document.getElementById("cfg-commit-source-cli");
    var apiPanel = document.getElementById("cfg-commit-api-panel");
    var cliPanel = document.getElementById("cfg-commit-cli-panel");
    var refresh = document.getElementById("cfg-commit-model-refresh");
    ...
    if (!status) return;
    var baseUrl = (document.getElementById("cfg-system-ai-base-url")?.value || "").trim();
    var model = (document.getElementById("cfg-system-ai-model")?.value || "").trim();
    var key = document.getElementById("cfg-system-ai-key");
    ...
    status.textContent = ready ? "\u5DF2\u5C31\u7EEA\uFF1A" + model + (host ? " \xB7 " + host : "") : baseUrl && !validUrl ? "\u5C1A\u672A\u5C31\u7EEA\uFF1AAPI \u5730\u5740\u5FC5\u987B\u662F\u6709\u6548\u7684 http(s) URL\u3002" : "\u5C1A\u672A\u5C31\u7EEA\uFF1A\u8BF7\u5148\u586B\u5199 API \u5730\u5740\u3001API Key \u548C\u6A21\u578B\u3002";
  }
  function organizeSettingsAiPanel() {
  ...
  function getActiveConfigMessage() {
    return document.querySelector(".settings-panel.active .settings-status-message") || document.getElementById("config-message");
  }
  var configSavePending = false;
  function fillSystemAiSettings(systemAi) {
    systemAi = systemAi || {};
    var enabled = document.getElementById("cfg-system-ai-enabled");
    var protocol = document.getElementById("cfg-system-ai-protocol");
    var authHeader = document.getElementById("cfg-system-ai-auth-header");
    var baseUrl = document.getElementById("cfg-system-ai-base-url");
    var model = document.getElementById("cfg-system-ai-model");
    var key = document.getElementById("cfg-system-ai-key");
    ...
    if (status) status.textContent = systemAi.source && systemAi.source !== "custom" ? "\u5DF2\u4ECE " + (systemAi.source === "claude" ? "Claude Code" : systemAi.source === "codex" ? "Codex" : "OpenCode") + " \u5BFC\u5165\u3002" : "\u63A7\u5236\u63D0\u793A\u8BCD\u4F18\u5316\u548C\u4F1A\u8BDD\u6807\u9898\uFF1BCommit \u6765\u6E90\u5728\u4E0B\u65B9\u5355\u72EC\u9009\u62E9\u3002";
    syncCommitSourceUI();
  }
  function importSystemAiConfig() {
    var button = document.getElementById("cfg-system-ai-import");
    ...
    fetch("/api/settings/system-ai/import", {
      ...
      headers: { "Content-Type": "application/json" },
      credentials: "same-origin",
      body: JSON.stringify({ source: document.getElementById("cfg-commit-cli")?.value || "claude" })
    }).then(function(res) {
      return res.json().then(function(data) {
      ...
      if (message) {
        message.textContent = "API \u914D\u7F6E\u5DF2\u4ECE CLI \u5BFC\u5165\u5E76\u4FDD\u5B58\uFF1B\u7CFB\u7EDF AI \u5F00\u5173\u548C Commit \u6765\u6E90\u4FDD\u6301\u4E0D\u53D8\u3002";
        message.style.color = "var(--success)";
        message.classList.remove("hidden");
      }
    }).catch(function(error) {
      if (message) {
        message.textContent = error.message || "\u5BFC\u5165\u5931\u8D25\u3002";
  ...
  function updateSettingsSidebarStatus(data) {
    if (!data) return;
    var cfg = data.config || {};
    var commitSourceLabel = cfg.commitAiSource === "api" ? "Commit \xB7 \u76F4\u8FDE API" : "Commit \xB7 " + (cfg.commitCli === "codex" ? "Codex" : cfg.commitCli === "opencode" ? "OpenCode" : "Claude");
    var metaMap = {
  ...
  function loadSettingsData() {
    ...
    fetch(settingsUrl, { credentials: "same-origin" }).then(function(res) {
      ...
      var commitSourceApiEl = document.getElementById("cfg-commit-source-api");
      var commitSourceCliEl = document.getElementById("cfg-commit-source-cli");
  ...
  function saveConfigSettings() {
    ...
    var systemAiKey = ((document.getElementById("cfg-system-ai-key") || {}).value || "").trim();
    var systemAiKeyEl = document.getElementById("cfg-system-ai-key");
    var commitAiSource = document.getElementById("cfg-commit-source-api")?.checked === true ? "api" : "cli";
    var systemAiEnabled = document.getElementById("cfg-system-ai-enabled")?.checked === true;
    var systemAiBaseUrlEl = document.getElementById("cfg-system-ai-base-url");
    var systemAiModelEl = document.getElementById("cfg-system-ai-model");
    ...
    if (savingAi && (commitAiSource === "api" || systemAiEnabled)) {
      var missingField = !systemAiBaseUrlEl?.value.trim() ? systemAiBaseUrlEl : !systemAiModelEl?.value.trim() ? systemAiModelEl : !(systemAiKey || systemAiKeyEl?.dataset.hasApiKey === "true") ? systemAiKeyEl : null;
      if (missingField) {
    ...
    var body = savingAi ? {
      ...
      systemAi: {
        ...
        protocol: (document.getElementById("cfg-system-ai-protocol") || {}).value || "openai",
        baseUrl: ((document.getElementById("cfg-system-ai-base-url") || {}).value || "").trim(),
        apiKey: systemAiKey,
        model: ((document.getElementById("cfg-system-ai-model") || {}).value || "").trim(),
        authHeader: document.getElementById("cfg-system-ai-auth-header")?.value === "x-api-key" ? "x-api-key" : "bearer",
    ...
    for (var saveIndex = 0; saveIndex < saveButtons.length; saveIndex++) {
      var pendingButton = saveButtons[saveIndex];
    ...
    fetch("/api/settings/config", {
      ...
      if (!data || !data.error) {
        ...
        if (aiTabMeta) {
          ...
          aiTabMeta.textContent = savedSource === "api" ? "Commit \xB7 \u76F4\u8FDE API" : "Commit \xB7 " + (savedCli === "codex" ? "Codex" : savedCli === "opencode" ? "OpenCode" : "Claude");
        }
      }
    }).catch(function() {
      if (msgEl) {
      ...
      for (var saveIndex2 = 0; saveIndex2 < saveButtons.length; saveIndex2++) {
        var pendingButton2 = saveButtons[saveIndex2];
        pendingButton2.disabled = false;
        pendingButton2.textContent = pendingButton2.dataset.saveLabel || "\u4FDD\u5B58\u914D\u7F6E";
        delete pendingButton2.dataset.saveLabel;
      }
    });
  }
