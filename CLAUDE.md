...
项目包含一个 Android **原生客户端**（Kotlin + Jetpack Compose，会话列表 / 聊天 / 新建会话 / 权限审批 / 设置直连 REST + `/ws`，对称 iOS 原生客户端；WebView 的 MainActivity 仅作设置页「网页版」兜底入口），源码在 `android/` 目录（git submodule → `co0ontty/wand-android`）。原生层结构：`data/`（WandHttp OkHttp 单例 + WandModels org.json 容错解析 + WandApi + WandSocket，合流/重连语义逐行对齐 `ios/Wand/ChatStore.swift` 与 `WandSocket.swift`）、`ui/`（ChatStore 状态机 + Compose 屏幕）。原有 Java 辅助类（ServerStore / NotificationHelper / UpdateManager / WandForegroundService / QR 扫码）原样复用，更新检查迁到 HomeActivity 启动时，图标切换抽取为 `AppIconSwitcher` 供原生与 WebView 桥共用。`updateSessionProgress` 网页驱动的进度通知与任务完成通知（sendNotification）在原生主路径下暂缺，待由原生 WS/前台服务驱动（v1.1）。聊天输入栏带**按住说话端侧语音识别**（`speech/` 模块）：sherpa-onnx 优先（按需下载，hf-mirror 镜像优先，下载到 `filesDir/asr/`）、系统 SpeechRecognizer 兜底（国产无 GMS ROM 上普遍不可用，所以 sherpa 是主路径）。sherpa 模型是注册表双选（设置页「语音输入」切换）：中文 CTC 小模型（约 26MB，默认）/ **中英混合 transducer 大模型**（`streaming-zipformer-bilingual-zh-en-2023-02-20` int8，约 190MB，code-switching 训练）——后者带程序员热词增强（`modified_beam_search` + `modelingUnit=cjkchar+bpe` + 内置 GIT/NPM/DOCKER 等约 130 词热词表，`SherpaSpeechEngine.DEV_HOTWORDS`）和英文全大写输出→小写后处理；所选模型未就绪时自动回退任一已就绪模型；`app/libs/` 里**提交了** sherpa-onnx static-link AAR（38MB，JitPack 对 k2-fsa 新 tag 构建失败所以钉本地文件），`abiFilters` 仅 arm64-v8a、`useLegacyPackaging=true` 压缩 .so（APK 下载体积约 +9MB）。
...
移动端客户端（尤其 Android）后续开发应优先参考 iOS 已验证布局与 `android/docs/ios-mobile-updates-reference-2026-06-18.md`。这份参考文档记录了 iOS 侧 PTY 原生壳、原生输入、快速提交入口、设置页外观选择等改动；Android 同步时不要另起一套交互模型，先对齐 iOS。

**本轮 Android 已同步/确认的规范：**
- PTY 页面采用原生外壳：原生顶栏 + 深色终端 WebView + 原生底部输入栏；WebView URL 使用 `embed=terminal&nativeInput=1`，网页输入栏隐藏。
- PTY 输入发送必须先发文本，再发送 `"\r"`，并带 `shortcutKey = "enter_text"`；不要用 `text + "\n"` 代替回车。
- Chat / PTY 的快速提交入口共用 `GitChangesButton` / `QuickCommitStore` / `QuickCommitSheet`；bottom sheet 允许半展开并保留默认 drag handle。
- 外观设置使用 `wand.appearanceMode` 持久化，值为 `light` / `dark` / `system`，设置页提供明亮 / 黑暗 / 跟随系统，`WandTheme` 按该模式决定暗色。
- Android 会话列表 header 参考 iOS inline toolbar：中间是分段切换（进行中 / 历史会话），左上是更多菜单，右上是新建或清空历史；不要恢复大标题式 header。
- 会话卡片布局：左侧 provider logo 在上，`终端` / `聊天` 胶囊在左下；右侧标题在上，路径在下，路径与左侧胶囊垂直居中对齐；时间胶囊在右上角。
- 时间胶囊只显示一个最大合适单位：不足 1 分钟为 `刚刚`，60 分钟内为 `N分钟`，24 小时内为 `N小时`，之后为 `N天`。
- 卡片视觉保持低噪声：深色中性表面、轻描边、8-14dp 圆角、克制阴影；不要回到厚重圆底按钮、强玻璃高光或大面积单一色系。
- Android 嵌入终端的“乱码”多数是列宽/字体问题，不是 UTF-8 解码问题。`embed=terminal` 下应使用较小等宽字号、收紧 padding，并在注入 CSS 后触发终端重新 fit；缺字形时优先加 Android 可用的 mono/symbol 字体兜底。
...
然后在已连接真机上截图检查 header、会话卡片、PTY 终端和输入栏。若设备上已装更高 `versionCode` 的包，调试安装需要 `-d`。
