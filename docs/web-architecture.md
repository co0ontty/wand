...
- 看板新建任务未选项目时，「归属父任务」可搜索所有**处理中**的任务，选择后带出项目与迭代；
  已有任务只列出同项目目录、处理中任务；
  父任务详情可直接「派发子任务」（预选父任务与项目，创建到「处理中」后启动独立 Agent 会话）。
  已有任务先选父任务，再从自身详情派发 Agent；每个子任务保留自己的会话、状态与侧栏工作任务，
  父任务详情列出子任务，卡片和列表展示归属。
- `wand_tasks.parent_task_id` 只加列，历史行仍为顶层任务。API 拒绝循环、跨项目与认领已完成的父任务；
  父任务后来完成/归档不抹掉原有归属。换项目先解绑父子归属，归档不删子任务及会话。
